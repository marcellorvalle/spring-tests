...
public class SingleClassSpecificationToolsTest {
    ...
    public void testLambdaMetafactory() {
        ConcreteReflective filter = Mockito.spy(new ConcreteReflective());

        findAll(filter.apply(props));

        Mockito.verify(filter).filterByName(props.get("name").get(0));
        Mockito.verify(filter).filterById(Long.valueOf(props.get("id").get(0)));
        Mockito.verify(filter).filterByFoo(props.get("foo"));
        Mockito.verify(filter).filterByBar(props.get("bar"));
        Mockito.verify(filter).filterByQux(props.get("qux"));
    }
